...
vector<int> BannedForBullet{1,2,16,17,31,30,207,ammoSymbol,healthSymbol,spikerSymbol,spergSymbol,chargerSymbol};
...
vector <string> settingsOptions{"Tick Speed","Horizontal Spacing","Vision Range","Delete Maps","Debug Hud","Resolution","Exit"};
...
vector <string> gameInfo{"Game by Bruno Ratajczyk"," ","Use W,S,A,D to move","Press X + W/S/A/D to shoot","Enemies: Sperk/Spiker/Charger ","Sperk: Moves Randomly","Spiker: Shoots when sees player","Charger: Charges at player","Pickups: heart/box","Heart: Adds one health","Box: Adds one ammo"};
vector <string> editorInfo{"Press L to wall corners","Press O-P to change vision","Press Enter to save","Press U to set Spawn Point"};
vector <string> resolutions{"1920x1080","1280x1024"};
vector <string> state{"Off","On"};

vector<string>Title{"C","O","L","O","R","Z","I","E"};
vector<string>Buttons{"W","S","A","D","F","G","H","V","B","N","enter","O","P","I","U","L","space","E"};
...
void KeyReset(){Sleep(300);while(kbhit()) getch();};
void ClearPlate(){ClearConsole();KeyReset();}
...
if(place<planetSizeX||place%planetSizeX==0||place%planetSizeX==planetSizeX-1||place>(planetSizeX*planetSizeY)-planetSizeX)
    {
     return true;
    }
    else return false;
...
for(int e=0;e<EnemySymbols.size();e++)
{if(planet[place]==EnemySymbols[e])return true;}
return false;}
...
int value=0;
while(value==0){
PushRenderWindow();cout<<"    "<<name<<endl<<endl;
PushRenderWindow();cout<<"  From "<<min<<" to "<<max<<endl;
PushRenderWindow();cout<<"    -> ";SetColor(240);
cin>>value;
ResetColor();
...
else if (ButtonClicked("space")&&pointer==0){ClearPlate();value=InputValue(name,min,max);break;}
...
PushRenderWindow();cout<<"    "<<name<<endl<<endl;
PushRenderWindow();cout<<"  From "<<min<<" to "<<max<<endl;
PushRenderWindow();cout<<"    < "<<value<<" >"<<endl<<endl;
PushRenderWindow();if(pointer==0){SetColor(240);}else cout<<"  ";cout<<"     Input Value"<<endl;
PushRenderWindow();if(pointer==1){SetColor(240);}else cout<<"  ";cout<<"     Accept";
ResetColor();
...
int  PickItemFromListV2(vector <string> itemList,string nameOfWindow,vector <string> values,bool lastSpacer,vector <string> coloredTitle,int startFrom){
...
LowerRenderWindow();
 ...
 else{for(int c=0;c<coloredTitle.size();c++){
 SetColor(rand()%256);
 cout<<coloredTitle[c]; }
...
PushRenderWindow();
int i=0;
if(accept){cout<<"   ";ColoredString(112,"Ok");while(i==0){if(ButtonClicked("space"))i=1;}}
else Sleep(time);
ClearConsole();
//Sleep(300);
}
...
ClearConsole();
int color=7;
DisplayColorPallete();KeyReset();
cout<<endl<<endl;PushRenderWindow();
cout<<"Pick Color: ";
cin>>color;
ResetColor();ClearPlate();
return color;
}
...
int  BulletDirection(int place){
        for(int b=0;b<4;b++)
        {
        if(planet[place]==char(bulletArrows[b]))
        {return Moves[b];}
        }
...
for(int b=0;b<BulletPlacement.size();b++)
   {
    ...
    if(planet[BulletPlacement[b]]!=' ')
    {
        ...
        //Check if can proceed
        if(IsNotBannedBulletObject(BulletPlacement[b]+direction)&&IsPoitingAtEdge(BulletPlacement[b],direction)==false&&IsABullet(BulletPlacement[b]+direction)==false)
        {
...
void EnemySpikerLogic(){
    for(int e=0;e<EnemySpiker.size();e++)
    {
        ...
        else
        {
            ...
            if(direction!=0&&PlayerAround(EnemySpiker[e])==false&&EnemySpikerShootDelay[e]==0&&IsEnemy(direction+EnemySpiker[e])==false)
            {
...
switch(type){
case 0:{planet[place]=spergSymbol;EnemySperg.push_back(place);break;}
case 1:{planet[place]=spikerSymbol;EnemySpiker.push_back(place);break;}
case 2:{planet[place]=chargerSymbol;EnemyCharger.push_back(place);break;}
}
...
cout<<endl;
if(debugHud){
    PushRenderWindow();cout<<"Tick: "<<tick<<endl;
    PushRenderWindow();cout<<"Position: "<<playerPosition<<endl;
    PushRenderWindow();cout<<"On Edge: "<<CheckIfOnEdge(playerPosition)<<endl;
    PushRenderWindow();cout<<"Enemy Amount: "<<EnemySperg.size()<<endl;
    PushRenderWindow();cout<<"C: "<<EnemyCharger.size()<<" Se: "<<EnemySperg.size()<<" Sp: "<<EnemySpiker.size()<<endl;
    PushRenderWindow();cout<<"PRW: "<<pushRenderStart<<endl;
}
 PushRenderWindow();for(int h=0;h<health;h++)ColoredChar(4,3);cout<<endl;
 PushRenderWindow();for(int a=0;a<playerMaxBullets-BulletPlacement.size();a++)ColoredChar(8,17);cout<<endl;
...
Map<<vision<<" "<<storedPlayerPosition<<endl;
    ...
    switch(planet[m]){
    case ' ':Map<<'0';break;
    case spergSymbol  :Map<<'s';break;
    case 1  :Map<<'p';break;
    case spikerSymbol :Map<<'x';break;
    case chargerSymbol :Map<<'c';break;
    default: Map<<planet[m];break;
    }}
...
storedPlayerPosition=playerPosition;
    ...
    Map>>symbol;
    switch(symbol){
    case'0':planet.push_back(' ');break;
    case's':EnemySperg.push_back(m);planet.push_back(spergSymbol);break;
    case'p':planet.push_back(1);break;
    case'x':EnemySpiker.push_back(m);planet.push_back(spikerSymbol);break;
    case'c':EnemyCharger.push_back(m);planet.push_back(chargerSymbol);break;
    default:planet.push_back(symbol);break;
    }}
...
int mapId;
switch(pointer){
case 1:SaveMap(mapAmount+1);CountMapAmount(true,false);     break;
case 2:mapId=SetValue("Override Map",1,1,1,mapAmount);SaveMap(mapId);break;
case 3:ClearPlate();break;
}
...
if(ButtonClicked("enter")){editorRunning=false;SaveMode();}
...
isWindowRendered=true;
PushRenderWindow();cout<<"Position: "<<playerPosition<<endl;
PushRenderWindow();for(int s=0;s<3;s++){cout<<symbolButtons[s]<<": "<<editorSymbol[s]<<" ";}cout<<endl;
PushRenderWindow();for(int c=0;c<3;c++){cout<<colorButtons[c]<<": ";SetColor(editorColor[c]);cout<<" ";ResetColor();cout<<" ";}cout<<endl;
PushRenderWindow();cout<<"Press I to add Enemy: ";ColoredChar(112,EnemySymbols[enemyToSpawn]);cout<<endl;
PushRenderWindow();cout<<"Press K for more information"<<endl;
isWindowRendered=false;
}
...
switch(pointer){
case 1:planetSizeX=SetValue("Map Width",planetSizeX,10,5,1000);break;
case 2:planetSizeY=SetValue("Map Height",planetSizeY,10,5,1000);break;
case 3: {int maxVision=planetSizeX;
        if(planetSizeX>planetSizeY)maxVision=planetSizeY;
        vision=SetValue("Vision Distance",vision,2,3,maxVision);break;}
case 4: whileOn=false;break;
}
}
...
void GameLoopEnd(){
    ...
    if(ButtonClicked("enter")){ClearPlate;gameRunning=false;}
    if(ButtonClicked("I")){hZ=FlipInt(hZ);if(hZ==1)pushRenderStart-=(vision+1)/2;if(hZ==0)pushRenderStart+=(vision+1)/2;}
    if(planetColor[playerPosition]==128&&gameRunning==false){ClearPlate();InfoScreen("You won",2000,false,eV);};
    ChangeFontModule();
    ClearConsole();
    tick++;
}
...
switch(pointer){
    case 1:NewMapCreation();editorRunning=true;break;
    case 2:LoadMode();editorRunning=true;break;
    case 3:break;
    }
...
while(editorRunning){
    ...
    else if(IsAnyKeyPressed()){refresh=true; ClearConsole();}
    }
}
void StartGame(){
    CountMapAmount(false,false);
...
switch(pointer){
case 1:InfoScreen("Game",0,true,gameInfo);
case 2:InfoScreen("Editor",0,true,editorInfo);
case 3:infoRun=false;break;
}
KeyReset();
}
KeyReset();
...
vector <string> optionsValues{to_string(tickSpeed),state[hZ],to_string(vision)," ",state[debugHud],resolutions[resolution]," "};
int pointer=PickItemFromListV2(settingsOptions,"Settings",optionsValues,true,eV,5);
switch(pointer){
case 1:tickSpeed=SetValue("Game Speed",tickSpeed,100,0,2000);break;
case 2:       hZ=SetValue("Horizontal Spacing",hZ,1,0,1);    break;
case 3:   vision=SetValue("Vision Range",vision,2,3,51);     break;
case 4:RemoveMaps();break;
case 5:debugHud=FlipBool(debugHud);        break;
case 6:resolution=FlipInt(resolution);break;
case 7:SaveSettings();whileOn=false;break;
}
}
}
void Quit(){
